...
def render_catalog_page(dist_root: Path) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        differences = (
            index_differences(dist_root, args.name) + lock_differences(dist_root) + catalog_page_differences(dist_root)
        )
        if differences:
    ...
    write_generated(dist_root / CATALOG_PAGE, render_catalog_page(dist_root))
