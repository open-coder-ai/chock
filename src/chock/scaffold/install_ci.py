...
Usage:
  chock sync --ci [repo] [--path .github/workflows/chock.yml]

`GITHUB_BASE_REF` and `GITHUB_HEAD_REF` are set automatically by Actions on `pull_request`
runs, so the generated step needs no repo-specific configuration -- it discovers every
compiled `ci-gate/gate.json` at run time, so a policy added later is covered without
...
#: Presence of this line is how a re-run tells "ours, safe to refresh" from "an adopter's
#: unrelated workflow that happens to sit at the same path" -- overwriting the latter would
#: silently discard someone else's CI configuration, which no installer in this project does.
...
#: Default location `compiler.py` checks when deciding whether ci-gate coverage is honest.
#: A custom `--path` is not detected there -- same tradeoff `.claude/settings.json` makes
#: for PreToolUse: a fixed, documented location is what makes "is this installed" a
#: question about repo contents rather than about install-time bookkeeping nothing kept.
