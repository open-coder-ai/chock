...
2. no answer on a fresh repo → default trio: claude, copilot, gemini (only claude gets a
   dedicated file -- copilot and gemini read `AGENTS.md` natively; see
   `references/adapters.md`). No answer on a *re-run* is different: it keeps whatever
   `supported_agents` is already in `.chock/config.yaml`, not the default trio.
...
2. existing `AGENTS.md`? init preserves it and manages only its marker section
   (`<!-- agentseam:begin -->`/`<!-- agentseam:end -->` markers) — confirm nothing
   hand-written will be lost
