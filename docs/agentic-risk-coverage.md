...
| `enforced-in-ci` | A CLI subcommand run as its own CI step and marked a required status check — re-derives the claim server-side, but is **not** a compiled `git-hook`/`ci-gate` artifact and carries no commit-time slice at all. Depends on the same branch-protection setting `ci-gate` does; see [the branch-protection gap](adopting.md#the-branch-protection-gap) |
